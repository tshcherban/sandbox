...
{
    uint8_t *byte = &_screenBuffer[y/8*84+x];
    if (value)
    *byte |= (1 << (y % 8));
    else
    *byte &= ~(1 << (y %8 ));
...
void nokia_lcd_write_char(char code, uint8_t scale)
{
    ...
    nokia_lcd_set_pixel(cursor_x + x, cursor_y + y, 1);
    else
    nokia_lcd_set_pixel(cursor_x + x, cursor_y + y, 0);
